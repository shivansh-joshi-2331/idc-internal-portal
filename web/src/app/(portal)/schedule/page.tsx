...
export default function SchedulePage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:4000/api/schedule?year=${year}&month=${month + 1}`, {
      ...
      const tRes = await fetch(`http://localhost:4000/api/time?year=${year}&month=${month + 1}&viewMode=${viewMode}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  useEffect(() => {
    if (activeDay) {
      ...
      fetch(`http://localhost:4000/api/time?date=${activeDay}&viewMode=${viewMode}`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(res => res.json())
        .then(data => {
  ...
  const deleteTask = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await fetch(`http://localhost:4000/api/time/${id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const submitLeaveRequest = async (e: React.FormEvent) => {
      e.preventDefault();
      try {
        const res = await fetch("http://localhost:4000/api/schedule/leave", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
